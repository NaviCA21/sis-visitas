    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            @if (session()->has('message'))
                <div class="bg-teal-100 rounded-b text-teal-900 px-4 py-2 shadow-md my-1">
                    <div class="flex items-center">
                        <div>
                            <h4 class="text-lg font-bold">{{ session('message') }}</h4>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="text-center">
                <div class="btn-group m-3 col-md-3">
                    <a class="btn {{ request()->is('visitas') ? 'btn-primary' : 'btn-info' }}"
                        href="{{ route('visitas.index') }}">Persona natural</a>
                    <a class="btn btn-info" href="{{ route('visitasjuridica.index') }}">Persona juridica</a>
                </div>
            </div>

            {{-- <button wire:click="crear()" class="btn btn-primary">Abrir modal</button> --}}
                                    ...
                                    @if ($visita->visitante->TipoVisitante->tipo_visitante == 'Persona Natural')
                                        <tr>
                                            ...
                                            <td class="border px-4 py-2">{{ $visita->visitante->nombre }}
                                                {{ $visita->visitante->a_paterno }} {{ $visita->visitante->a_materno }}
                                            </td>
                                            ...
                                            <td class="border px-4 py-2">
                                                {{ $visita->periodo->hora_inicio . ' - ' . $visita->periodo->hora_fin }}
                                            </td>
